...
class Wcontrolbutton(Button):
  def __init__(self, parent, *args, **kwargs):
    ...
    fonts= parent.fonts
    self.config(font= fonts['controlbuttonfont'])
...
class Wentry(Entry):
  def __init__(self, parent, root, *args, **kwargs):
    palette= self.palette= parent.palette
    fonts= parent.fonts;  entryfont= fonts['entryfont']
...
class Woptionmenu(OptionMenu):
  ...
  def __init__(self, parent, optionlist, val= '', default= None, **kwargs):
    #print('Woptionmenu.__init__:', str(val))
    self.optionlist= optionlist;  self.default= default
    if val== '' and default!= None:  val= default
    sv= StringVar();  sv.set(str(val));  self.sv= sv
    super().__init__(parent, sv, *self.optionlist)

  def gets(self):
     s= self.sv.get()
     if not s or s== '':  return self.default
     return str(s)
  def get(self):      return self.gets()
  def getint(self):   return int(self.gets())
  def set(self, val):  self.sv.set(str(val));  #print('Woptionmenu.set:', str(val))
...
class widgets2dict:
  def __init__(self, dict= {}, notouch= None):
    self.dict= dict;  self.notouch= notouch;  self.assocs= []
  ...
  def dict2widgets(self, adict= None):
    if not adict:  adict= self.dict;  assert adict
